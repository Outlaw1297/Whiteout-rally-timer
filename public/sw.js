self.__RALLY_SW_VERSION = "2026-08-16-origin-guard-1";
...
/**
 * Notification URLs are generated by a background server that might only know
 * its internal Render address (for example https://localhost:10000). Never let
 * a remote payload navigate outside the installed PWA's actual origin.
 */
function safeNotificationNavigate(value, fallbackPath = "/caller") {
  const origin = self.location.origin;
  let fallback;
  try {
    fallback = new URL(fallbackPath, origin).toString();
  } catch {
    fallback = `${origin}/caller`;
  }
...
self.addEventListener("push", (event) => {
  ...
  const fallbackPath =
    assignmentId && rallyId && !String(rallyId).startsWith("calibration")
      ? `/caller/events/${encodeURIComponent(rallyId)}`
      : "/caller";
  const url = safeNotificationNavigate(data.navigate, fallbackPath);
...
self.addEventListener("notificationclick", (event) => {
  ...
  const url = safeNotificationNavigate(
    event.notification.data?.url || event.notification.data?.navigate,
    "/caller"
  );
